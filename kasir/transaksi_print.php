 ...
 <body style="padding: 30px; font-size: 23px;">
 	...
 	if($_GET['id']) { ?>
		...
		include "../koneksi.php";
		$id 		= $_GET['id'];
 		$penjualan 	= mysqli_query($koneksi,"SELECT penjualan.*, user.id_user, user.nama_lengkap FROM penjualan INNER JOIN user ON penjualan.id_user = user.id_user WHERE id_penjualan = '$id'");
